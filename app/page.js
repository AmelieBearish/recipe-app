      ...
      {!loading && featuredRecipe && (
        <div className="mb-5 rounded-2xl overflow-hidden border border-amber-200 bg-amber-50 shadow-sm">
          <div className="flex items-stretch cursor-pointer" onClick={() => router.push(`/recipes/${featuredRecipe.id}`)}>
            <div className="w-28 h-28 flex-shrink-0 relative bg-amber-100">
            ...
            <div className="flex flex-col justify-center px-4 py-3 flex-1 min-w-0">
              <span className="text-xs font-bold text-amber-600 mb-1">✨ 今日のおすすめ</span>
              <p className="font-bold text-gray-800 text-base leading-snug line-clamp-2">{featuredRecipe.title}</p>
              {featuredRecipe.description && (
                <p className="text-xs text-gray-500 mt-1 line-clamp-2">{featuredRecipe.description}</p>
              )}
            </div>
          </div>
